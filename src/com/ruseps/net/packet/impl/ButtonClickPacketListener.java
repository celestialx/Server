...
import com.ruseps.world.content.teleportation.TeleportBosses;
...
import com.ruseps.world.content.teleportation.teleportsystem.BossInformationEnum;
import com.ruseps.world.content.teleportation.teleportsystem.DungInformation;
import com.ruseps.world.content.teleportation.teleportsystem.MinigameInformation;
...
public class ButtonClickPacketListener implements PacketListener {
	...
	public void handleMessage(Player player, Packet packet) {
		...
		if (id >= 155151 && id <= 155156) {
			MinigameInformation.handleInformation(id, player);
		}
		
		if (id >= 151175 && id <= 151178) {
			DungInformation.handleInformation(id, player);
		}
		
		if (id >= -28705 && id <= -28681) {
		...
		if (id == -4279 && player.lastClickedTab != 4) {
			LmsHandler.joinLobby(player);

		}
		
		if (id == -28725) {
			if (player.bossInterface == -28691)  {
				TeleportHandler.teleportPlayer(player, new Position(3696, 5807, player.getIndex() * 4),
						player.getSpellbook().getTeleportType());
		
			} else {
			BossInformation.handleInformationTele(player.bossInterface, player);
			}
		}
...
break;
		...
		case -12319:
			if(!player.loading) {
				player.loading = !player.loading;
				player.getAchievements().tab(1,false);
				player.getPA().sendFrame126(53204, "CelestialX Achievements - Sorted: Easy");
			}
			break;
		case -12315:
			if(!player.loading) {
				player.loading = !player.loading;
				player.getAchievements().tab(2,false);
				player.getPA().sendFrame126(53204, "CelestialX Achievements - Sorted: Medium");
			}
			break;
		case -12311:
			if(!player.loading) {
				player.loading = !player.loading;
				player.getAchievements().tab(3,false);
				player.getPA().sendFrame126(53204, "CelestialX Achievements - Sorted: Hard");
			}
			break;
		case -12307:
			if(!player.loading) {
				player.loading = !player.loading;
				player.getAchievements().tab(4,false);
				player.getPA().sendFrame126(53204, "CelestialX Achievements - Sorted: Elite");
			}
			break;
		case -12303:
			player.getAchievements().claim();
			break;
		case -12331:
			player.getAchievements().sortByStyle("PvM");
			player.getPA().sendFrame126(53204, "CelestialX Achievements - Sorted: Pvm");
			break;
		case -12327:
			player.getAchievements().sortByStyle("PvP");
			player.getPA().sendFrame126(53204, "CelestialX Achievements - Sorted: Pvp");
			break;
		case -12323:
			player.getAchievements().sortByStyle("Skill");
			player.getPA().sendFrame126(53204, "CelestialX Achievements - Sorted: Skill");
			break;
		...
		case -26333:
			...
			player.getPacketSender().sendMessage("Attempting to open: CelestialX.org/forums");
			break;
		case -26332:
			player.getPacketSender().sendString(1, "www.CelestialX.org/rules/");
			player.getPacketSender().sendMessage("Attempting to open: CelestialX.org/rules");
			break;
		case -26331:
			player.getPacketSender().sendString(1, "www.CelestialX.org/store/");
			player.getPacketSender().sendMessage("Attempting to open: CelestialX.org/store");
			break;
		case -26330:
			player.getPacketSender().sendString(1, "www.CelestialX.org/vote/");
			player.getPacketSender().sendMessage("Attempting to open: CelestialX.org/vote");
			break;
		case -26329:
			player.getPacketSender().sendString(1, "www.CelestialX.org/hiscores/");
			player.getPacketSender().sendMessage("Attempting to open: CelestialX.org/hiscores");
			break;
		case -26328:
			player.getPacketSender().sendString(1, "www.CelestialX.org/report/");
			player.getPacketSender().sendMessage("Attempting to open: CelestialX.org/report");
			break;
		case 26750:
		...
		case 8672:
			...
			break;
		case 8861:
			TeleportHandler.teleportPlayer(player, new Position(2956, 2789), player.getSpellbook().getTeleportType());
			break;
		case 8656:
			TeleportHandler.teleportPlayer(player, new Position(2993, 2699), player.getSpellbook().getTeleportType());
			break;
		case 8659:
			TeleportHandler.teleportPlayer(player, new Position(2996, 2724), player.getSpellbook().getTeleportType());
			break;
		case 8664:
			TeleportHandler.teleportPlayer(player, new Position(2993, 2725), player.getSpellbook().getTeleportType());
			break;
		case 8666:
		 ...
		 */
			...
			/*
			 * should close the interface method, after its invoked.
			 */
		case -4334:
			player.getPacketSender().sendInterfaceRemoval();
			break;
		...
		case 10003:
			if (player.getLocation() == Location.FREE_FOR_ALL_WAIT
					|| player.getLocation() == Location.FREE_FOR_ALL_ARENA) {
				player.sendMessage("You cannot open this interface while in this area");
				return;
			} else {
...
//			break;
		...
		case 8668:
			...
			break;
		case 8665:
			TeleportHandler.teleportPlayer(player, new Position(2997, 2728), player.getSpellbook().getTeleportType());
			break;
		case 8662:
			TeleportHandler.teleportPlayer(player, new Position(3002, 2729), player.getSpellbook().getTeleportType());
			break;
		case 13928:
			TeleportHandler.teleportPlayer(player, new Position(2954, 2737), player.getSpellbook().getTeleportType());
			break;
		case 28179:
...
//			break;
		...
		case 26716:
			if (player.musicActive() == true) {
				...
				player.getPacketSender().sendString(26716,
						"@or2@Music:  @gre@" + (player.musicActive() ? "On" : "Off") + "");
			} else {
		...
		case 26717:
			if (player.soundsActive() == true) {
				...
				player.getPacketSender().sendString(26717,
						"@or2@Sounds:  @gre@" + (player.soundsActive() ? "On" : "Off") + "");
			} else {
