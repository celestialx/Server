...
import com.ruseps.world.content.skill.impl.construction.ConstructionActions;
...
import com.ruseps.world.content.minigames.impl.lms.LmsObjectHandler;
import com.ruseps.world.content.skill.impl.agility.Agility;
import com.ruseps.world.content.skill.impl.construction.Construction;
import com.ruseps.world.content.skill.impl.crafting.Flax;
import com.ruseps.world.content.skill.impl.crafting.Jewelry;
		...
		player.setInteractingObject(gameObject)
				.setWalkToTask(new WalkToTask(player, position, gameObject.getSize(), new FinalizedMovementTask() {
					...
					public void execute() {
						...
						LmsObjectHandler.handleObject(player, gameObject);
						
						if(LmsObjectHandler.handleCrateSearch(player, gameObject)) {
							return;
						}
						if (SearchScrolls.handleObject(player, position)) {
							return;
						}
						...
						switch (id) {
						case 26760:
							if (player.getPosition().getY() >= 2717) {
								player.setPositionToFace(new Position(2927, 1));
								player.moveTo(
										new Position(player.getPosition().getX(), player.getPosition().getY() - 1));
							} else {
								player.setPositionToFace(new Position(2927, 9000));
								player.moveTo(
										new Position(player.getPosition().getX(), player.getPosition().getY() + 1));
							}
						...
						case 77:
						case 78:
							if (player.getPosition().getX() == 2911 || player.getPosition().getX() == 2905
									|| player.getPosition().getX() == 2915) {
								player.setPositionToFace(new Position(9000, 2777));
								player.moveTo(
										new Position(player.getPosition().getX() + 2, player.getPosition().getY()));
							}
							if (player.getPosition().getX() == 2912 ||player.getPosition().getX() == 2906 || player.getPosition().getX() == 2916) {
								player.setPositionToFace(new Position(1000, 2777));
								player.moveTo(
										new Position(player.getPosition().getX() - 1, player.getPosition().getY()));
							}
							break;
						...
						case 29577:
							...
							break;
						case 18493:
							player.getPacketSender().sendString(1, "www.CelestialX.org/forum/index.php?/topic/25-map-guide/");
						...
						case 28714:
							player.performAnimation(new Animation(828));
							player.delayedMoveTo(new Position(3000, 2735), 2);
							break;
						case 1746:
							player.performAnimation(new Animation(827));
