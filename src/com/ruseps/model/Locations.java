...
public class Locations {
	...
	public enum Location {
		GENERAL(new int[] { 2890, 2906, 2885, 2911 }, new int[] { 2783, 2804, 2788, 2799 }, true, false, false, false, false,
				false) {

		},
		MUTANT(new int[] { 2994, 2970 }, new int[] { 3776, 3736 }, true, true, false, false, false,
				false) {

		},
		LEXICUS(new int[] { 2428, 2409 }, new int[] { 3533, 3520 }, true, true, false, false, false,
				false) {

		},
		LMS_GAME(new int[]{2689, 2826}, new int[]{2692, 2819}, true, false, true, false, false, false) {
			@Override
			public void process(Player player) {
				if (!player.walkableInterfaceList.contains(22119))
					player.sendParallellInterfaceVisibility(22119, true);
			}
			@Override
			...
			public void onDeath(Player player) {
				LmsHandler.leaveGame(player, "game");
			}

			@Override
			public boolean canAttack(Player player, Player target) {
				if (target.getLocation() != LMS_GAME) {
		...
		LMS_LOBBY(new int[] { 2448, 2479 }, new int[] { 4766, 4793 }, false, false, false, false, false, true) {
			@Override
			public void process(Player player) {
				if (!player.walkableInterfaceList.contains(22119))
					player.sendParallellInterfaceVisibility(22119, true);
			...
			/*	player.getPacketSender()
						.sendMessage("Teleport spells are blocked here, if you wish to teleport, use the portal.");*/
				LmsHandler.leaveGame(player, "lobby");
				return true;
			}

		},
		BOSS_GAME(new int[] { 1885, 1913 }, new int[] { 5340, 5369 }, true, true, false, false, false, false) {
		...
		FREE_FOR_ALL_ARENA(new int[] { 3250, 3350 }, new int[] { 9800, 9900 }, false, true, true, false, false, true) {
			@Override
			public boolean canTeleport(Player player) {
				player.getPacketSender()
						.sendMessage("Teleport spells are blocked here, if you wish to teleport, use the portal.");
		...
		FREE_FOR_ALL_WAIT(new int[] { 2505, 2545 }, new int[] { 4750, 4791 }, false, false, true, false, false, true) {
			@Override
			public boolean canTeleport(Player player) {
				player.getPacketSender()
						.sendMessage("Teleport spells are blocked here, if you wish to teleport, use the portal.");
		...
		/*
		 * FFALOBBY(new int[] { 2505, 2545 }, new int[] { 4750, 4791 }, false, false,
		 * true, false, true, true) {
		 * 
		 * @Override public boolean canTeleport(Player player) {
		 * player.getPA().sendInterfaceRemoval(); player.getPacketSender().
		 * sendMessage("You can't tele here. Use command ::ffaleave to leave"); return
		 * false; }
		 * 
		 * @Override public void login(Player player) { FreeForAll.leaveArena(player); }
		 * 
		 * @Override public void logout(Player player) { FreeForAll.leaveArena(player);
		 * }
		 * 
		 * @Override public void onDeath(Player player) { FreeForAll.leaveArena(player);
		 * }
		 * 
		 * }, FFAARENA(new int[] { 3250, 3350 }, new int[] { 9800, 9900 }, false, false,
		 * true, false, true, true) {
		 * 
		 * @Override public boolean canAttack(Player player, Player target) { return
		 * true; }
		 * 
		 * @Override public void process(Player player) { player.setWildernessLevel(60);
		 * 
		 * }
		 * 
		 * @Override public boolean canTeleport(Player player) {
		 * player.getPacketSender().sendInterfaceRemoval();
		 * player.getPacketSender().sendMessage("You can't teleport here"); return
		 * false; }
		 * 
		 * @Override public void login(Player player) { FreeForAll.leaveArena(player); }
		 * 
		 ...
		 * player.getPacketSender().sendInteractionOption("Attack", 2, true);
		 * player.getPacketSender().sendString(19000, "Combat level: " +
		...
		WILDERNESS(new int[] { 2941, 3392, 2986, 3012, 3653, 3710, 3650, 3653, 2962, 2984 },
				new int[] { 3523, 3968, 10338, 10366, 3441, 3538, 3457, 3472, 2696, 2716 }, false, true, true, true,
				true, true) {
			@Override
			public void process(Player player) {
				...
				boolean resource = x >= 2962 && x <= 2984 && y >= 2696 && y <= 2716;
				if (ghostTown) {
			...
			public void enter(Player player) {
				...
				if(player.getEquipment().contains(20690) || player.getEquipment().contains(995)) {
					player.getPacketSender().sendMessage("You cannot take prohibited Customs into the wilderness!");
					TeleportHandler.teleportPlayer(player, new Position(3093, 3503),
					player.getSpellbook().getTeleportType());
					return;
				}
			...
			public boolean canTeleport(Player player) {
				if (player.getWildernessLevel() > 20) {
					if (player.getRights() == PlayerRights.OWNER || player.getRights() == PlayerRights.DEVELOPER || player.getRights() == PlayerRights.ADMINISTRATOR) {
						player.getPacketSender()
