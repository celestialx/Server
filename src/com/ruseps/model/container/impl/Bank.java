...
public class Bank extends ItemContainer {
	...
	public Bank open(Player player, boolean reset) {
		if (player.getLocation() == Location.FREE_FOR_ALL_ARENA || player.getLocation() == Location.FREE_FOR_ALL_WAIT) {
			return this;
		}
		if(reset) {
		...
		if(player.getGameMode() == GameMode.HARDCORE_IRONMAN) {
			...
			player.getPacketSender().sendInterfaceRemoval().sendMessage("Hardcore-ironman-players cannot use banks.");
			return this;
		}
		}
		sortItems().refreshItems();
		player.setBanking(true).setInputHandling(null);
		player.getPacketSender().sendConfig(115, player.withdrawAsNote() ? 1 : 0).sendConfig(304, player.swapMode() ? 1 : 0).sendConfig(117, (player.getBankSearchingAttribtues().isSearchingBank() && player.getBankSearchingAttribtues().getSearchedBank() != null) ? 1 : 0).sendInterfaceSet(5292, 5063);
		return this;
	}
	...
	public static void sendTabs(Player player, Bank[] bankTabs) {
		...
		/*boolean moveRest = false;
		for(int i = 1; i <= 7; i++) {
			if(isEmpty(bankTabs[i)) || moveRest) {
				int j = i+2 > 8 ? 8 : i+2;
				player.setBank(i, bankTabs[j));
	...
	public static void depositItems(Player p, ItemContainer from, boolean ignoreReqs) {
		...
		if(p.getLocation() == Location.FREE_FOR_ALL_ARENA || p.getLocation() == Location.FREE_FOR_ALL_WAIT) {
			return;
		}
			
		for (Item it : from.getValidItems()) {
			if(p.getGameMode() == GameMode.HARDCORE_IRONMAN) {
				if(p.getBank(p.getCurrentBankTab()).getFreeSlots() <= 322 && !(p.getBank(p.getCurrentBankTab()).contains(it.getId()) && it.getDefinition().isStackable())) {
					p.getPacketSender().sendMessage("@red@Bank full. You only have 30 slots as a hc iron man for dungeoneering.");
				/*	Item toDrop = new Item(ItemDefinition.forId(it.getId()).isNoted() ? (it.getId() - 1) : it.getId(), it.getAmount());
					int drop = toDrop.getId();
					GroundItemManager.spawnGroundItem(p, new GroundItem(new Item(drop), p.getPosition().copy(), p.getUsername(), false, 110, true, 100));
*/
					return;
				}
			} 
	...
	public static class BankSearchAttributes {
		...
		public static void stopSearch(Player player, boolean openBank) {
			...
			player.setCurrentBankTab(0).setNoteWithdrawal(false);
			player.getPacketSender().sendString(27002, Integer.toString(0)).sendString(27000, "1").sendConfig(117, 0).sendString(5383, "        The Bank of CelestialX");
			if(openBank)
