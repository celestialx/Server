...
public final class RegionClipping {
  ...
  public static boolean objectExists(GameObject object) {
    ...
    boolean crystalChest =pos.getX() >= 0;
  ...
  public static boolean canMove(Position start, Position end, int xLength, int yLength) {
	...
	public boolean canMove(int x, int y, int z, int direction) {
		...
		if (direction == 0) {
			return !blockedNorthWest(p) && !blockedNorth(p) && !blockedWest(p);
		} else if (direction == 1) {
			return !blockedNorth(p);
		} else if (direction == 2) {
			return !blockedNorthEast(p) && !blockedNorth(p) && !blockedEast(p);
		} else if (direction == 3) {
			return !blockedWest(p);
		} else if (direction == 4) {
			return !blockedEast(p);
		} else if (direction == 5) {
			return !blockedSouthWest(p) && !blockedSouth(p) && !blockedWest(p);
		} else if (direction == 6) {
			return !blockedSouth(p);
		} else if (direction == 7) {
			return !blockedSouthEast(p) && !blockedSouth(p) && !blockedEast(p);
		}
		return false;
	}
  ...
  public static boolean blocked(Position pos, int size) {
