...
public class GrandExchange {
	...
	public static void updateViewPurchaseInterface(Player player, int boxIndex) {
		...
		player.getPacketSender().
		sendItemOnInterface(58780, geSlot.getOffer().getId(), 1).
		sendItemOnInterface(58781, -1, 1).
		sendItemOnInterface(58782, -1, 1).
		sendString(58769, def.getName()).
		sendString(58770, def.getDescription()).
		sendString(58771, geSlot.getOffer().getAmountFinished() > 0 ? (""+(geSlot.getOffer().getAmountFinished() >= geSlot.getOffer().getAmount() ? ("@gre@") : ("@yel@"))+"" + Misc.insertCommasToNumber(""+geSlot.getOffer().getAmountFinished()+"") + " / "+Misc.insertCommasToNumber(""+geSlot.getOffer().getAmount()+"")+"") : (Misc.insertCommasToNumber(""+geSlot.getOffer().getAmount()+""))).
		sendString(58772, ""+Misc.insertCommasToNumber(""+geSlot.getOffer().getPricePerItem()+"")+" gp").
		sendString(58773, ""+Misc.insertCommasToNumber(""+geSlot.getOffer().getTotalCost()+"")+" gp");
		if(geSlot.getState() == GrandExchangeSlotState.PENDING_PURCHASE || geSlot.getState() == GrandExchangeSlotState.FINISHED_PURCHASE || geSlot.getState() == GrandExchangeSlotState.ABORTED) {
			if(geSlot.getOffer().getAmountFinished() == 0) {
				if(geSlot.getOffer().getCoinsCollect() > 0) {
					player.getPacketSender().sendItemOnInterface(58781, 995, geSlot.getOffer().getTotalCost());
				}
			} else {
				if(geSlot.getOffer().getItemCollect() > 0) {
					...
					player.getPacketSender().sendItemOnInterface(58781, item, geSlot.getOffer().getItemCollect());
					if(geSlot.getOffer().getCoinsCollect() > 0) {
						player.getPacketSender().sendItemOnInterface(58782, 995, geSlot.getOffer().getCoinsCollect());
					}
				} else {
					if(geSlot.getOffer().getCoinsCollect() > 0) {
						player.getPacketSender().sendItemOnInterface(58781, 995, geSlot.getOffer().getCoinsCollect());
					}
				}
			}
		}
	...
	private static final int VIEW_PURCHASE_INTERFACE = 58700;
