...
public class Trading {
	...
	public void giveItems() {
		...
		try {
			...
			if (item.getId() == 995) {
				if (item.getAmount() >= 20000000) {
			        World.sendAdminMessage("@red@[ADMIN]@bla@ " +player.getUsername()+ " Gave "+player2.getUsername()+". Id: "+item.getId()+"(cash), amount: "+Misc.insertCommasToNumber(String.valueOf(item.getAmount())));
			...
			for (Item item : player2.getTrading().offeredItems) {
				PlayerLogs.log(player.getUsername(), "Receiving item from trade. Id: "+item.getId()+", amount: "+item.getAmount()+ " from: "+player2.getUsername() );
