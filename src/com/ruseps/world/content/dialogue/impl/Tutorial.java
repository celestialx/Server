	...
	public static Dialogue get(Player p, int stage) {
		...
		case 1:
			dialogue = new Dialogue() {
				...
				@Override
				public String[] dialogue() {
					return new String[]{"You can earn money doing many different things in", "CelestialX. For example, see those Thieving stalls infront of ", "you? You can steal items from them and sell them to the", "merchant whose standing over there."};
				};
		...
		case 6:
			dialogue = new Dialogue() {
				...
				@Override
				public String[] dialogue() {
					return new String[]{"Enough of the boring stuff, let's show you some creatures!", "There are a bunch of bosses to fight in CelestialX.", "Every boss drops unique and good gear when killed.", "One example is the mighty Pohenix!"};
				};
		...
		case 8:
			dialogue = new Dialogue() {
				...
				@Override
				public String[] dialogue() {
					return new String[]{"CelestialX also has a lot of enjoyable minigames.", "This is the Graveyard Arena, an area that's been run over", "by Zombies. Your job is to simply to kill them all.", "Sounds like fun, don't you think?"};
				};
		...
		case 11:
			dialogue = new Dialogue() {
				...
				@Override
				public String[] dialogue() {
					return new String[]{"CelestialX is a competitive game. Next to you is a scoreboard", "which you can use to track other players and their progress."};
				};
		...
		case 13:
			dialogue = new Dialogue() {
				...
				@Override
				public String[] dialogue() {
					return new String[]{"If you have any more questions, simply use the ::help", "command and a staff member should get back to you.", "You can also join the clanchat channel 'help' and ask", "other players for help there too. Have fun playing CelestialX!"};
				};
				...
				public void specialAction() {
					...
					if(ConnectionHandler.getStarters(p.getHostAddress()) <= GameSettings.MAX_STARTERS_PER_IP) {
						if(p.getGameMode() != GameMode.NORMAL) {
							p.getInventory().add(995, 10000).add(1153, 1).add(1115, 1).add(1067, 1).add(1323, 1).add(1191, 1).add(841, 1).add(882, 50).add(1167, 1).add(1129, 1).add(1095, 1).add(1063, 1).add(7394, 1).add(7390, 1).add(7386, 1).add(1379, 1).add(556, 50).add(558, 50).add(557, 50).add(555, 50).add(1351, 1).add(1265, 1).add(1712, 1).add(11118, 1).add(1007, 1).add(1061, 1).add(330, 100).add(1419, 1);
						} else {
					...
					World.sendMessage("<img=10> <col=6600CC>[NEW PLAYER]: "+p.getUsername()+" has logged into CelestialX for the first time!");
