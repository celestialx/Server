...
public class PrayerHandler {
	...
	public static void activatePrayer(Player player, final int prayerId) {
		...
		if(player.getLocation() == Location.RECIPE_FOR_DISASTER || player.getLocation() == Location.BOSS_GAME) {
	...
	private static void startDrain(final Player player) {
		...
		TaskManager.submit(new Task(1, player, true) {
			@Override
			public void execute() {
				//if((player.getRights() == PlayerRights.EXTREME_DONATOR || player.getRights() == PlayerRights.PLATINUM_MEMBER || player.getRights() == PlayerRights.DIAMOND_MEMBER) && player.getLocation() != Location.WILDERNESS)
				//	return;
				...
				/*if(player.getLocation() != Location.WILDERNESS) {
					if(player.getRights() == PlayerRights.EXTREME_DONATOR || player.getRights() == PlayerRights.PLATINUM_MEMBER || player.getRights() == PlayerRights.DIAMOND_MEMBER) {
						drainAmount = 0;
					}
				} */
				if (drainAmount <= 0) {
