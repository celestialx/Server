	...
	public void execute() {
		...
		try {
			...
			case 1:
				...
				if(loc == Location.LMS_GAME) {
					Player killer = player.getCombatBuilder().getKiller(true);
					LmsHandler.gameMessage(killer, player);
					}
				if(loc != Location.DUNGEONEERING && loc != Location.PEST_CONTROL_GAME && loc != Location.DUEL_ARENA && loc != Location.FREE_FOR_ALL_ARENA && loc != Location.FREE_FOR_ALL_WAIT && loc != Location.SOULWARS && loc != Location.FIGHT_PITS && loc != Location.FIGHT_PITS_WAIT_ROOM && loc != Location.FIGHT_CAVES && loc != Location.RECIPE_FOR_DISASTER && loc != Location.GRAVEYARD) {
					Player killer = player.getCombatBuilder().getKiller(true);
					if( player.getRights().equals(PlayerRights.OWNER) || player.getRights().equals(PlayerRights.DEVELOPER) || player.getRights().equals(PlayerRights.ADMINISTRATOR))
						dropItems = false;
					if(loc == Location.WILDERNESS) {
						if(killer != null && (killer.getRights().equals(PlayerRights.OWNER) || killer.getRights().equals(PlayerRights.DEVELOPER) || killer.getRights().equals(PlayerRights.ADMINISTRATOR)))
							dropItems = false;
					}
					...
					if(killer != null) {
						if(killer.getRights().equals(PlayerRights.OWNER) || killer.getRights().equals(PlayerRights.DEVELOPER) || killer.getRights().equals(PlayerRights.ADMINISTRATOR)) {
							dropItems = false;
						}
					}
